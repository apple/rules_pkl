...
def _pkl_package_impl(ctx):
    ...
    ctx.actions.run_shell(
        ...
        "$pkl" eval "$pkl_project" "${extra_args[@]:+${extra_args[@]}}" -x "package.name" > "$name_file"
        "$pkl" eval "$pkl_project" "${extra_args[@]:+${extra_args[@]}}" -x "package.version" > "$version_file"
        "$pkl" eval "$pkl_project" "${extra_args[@]:+${extra_args[@]}}" -x "package.baseUri" > "$base_uri_file"
        """,
        arguments = [metadata_eval_args],
        inputs = [pkl_project_symlink],
        outputs = [name_file, version_file, base_uri_file],
        tools = [executable],
    )
